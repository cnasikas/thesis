...
Various general function are provided to facilitate the user of the system.
...
When a data processor does the computation and generate the proof it has to sent a \verb|addProof| transaction, to store the proof and the encrypted output, with the following values:
...
Every function that is executed by a signed transaction triggers an event associated with the action. Every node of the network can register to listen to those events.
