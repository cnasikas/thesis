...
A PoW system is based on randomness. Each node has a small change to win the block which is approximated proportionally to the computational power of each node. To obtain control of the blockchain, an adversary would need to control the majority of the CPU power of the network. Bitcoin's security has been formalized and rigorously explored~\cite{10.1007/978-3-662-46803-6_10} over the years.
...
\begin{figure}[ht!]
  ...
  \begin{tikzpicture}
    \foreach \x in {0,1,...,4}
        \pgfmathparse{(\x*3)}
        \edef\position{\pgfmathresult}
        \node[rectangle,draw,minimum width=1.5cm,minimum height=1cm] (\x) at (\position,0) {};
    ...
    \foreach \x in {2,3,...,5}
        \pgfmathparse{(\x*3)}
        \edef\position{\pgfmathresult}
        \node[rectangle,draw,minimum width=1.5cm,minimum height=1cm, red] (\x_mal) at (\position,2) {};

    \draw [->] (3_mal) -- (2_mal);
    \draw [->] (4_mal) -- (3_mal);
    \draw [->] (5_mal) -- (4_mal);

    \draw [<-] (1.north) -- (2_mal.west);
