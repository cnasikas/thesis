...
Cyclic subgroups of such elliptic curve groups can be used to implement discrete logarithm systems.
...
The prime $p$, the curve $E$ the generator point $P$, its order $q$ and the public key $Q$ are public. The problem of finding $d$ given $Q$ and $P$ is called elliptic curve discrete logarithm problem (ECDLP) which is assumed to be hard.

\subsubsection{Diffie-Hellman key exchange}
\label{preliminaries:pub:el_curves:dh}

The elliptic curve analogue of Diffie-Hellman key exchange protocol defined in~\ref{preliminaries:pub:dh} can implemented as follows~\cite{elliptic_curves}:
...
\label{preliminaries:pub:el_curves:ecdsa}
