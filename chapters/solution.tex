...
\begin{algorithm}[!htb]
  ...
  \Procedure{\sf verify}{e}
    \Let{(requestID)}{e}
    \Let{tx}{(requestID)}
    \Let{(\pi, c, h)}{tx.send()} \Comment{Get request info}
    \Let{(\pi, (y, valid, s))}{Dec_{sk_{r}}(c)} \Comment{Decrypt results}
    \Let{result}{\_verify(\pi, y, valid, h, s, vk_f)} \Comment{Verify proof}
