...
As it has already been mentioned in our study, the role of a data processor is crucial. By task assignments, it decrease the pressure over data controllers and are exploited by them to entrust specific data processing operations on behalf of data requestors. Data processors can only perform pre-agreed algorithms (~\ref{solution:algorithms}) on datasets. The source code of the algorithms is crucial to be open-source as
...
\begin{algorithm}[!htb]
  ...
  \begin{algorithmic}[1]
  \Function{\sf compute}{$d$, algorithmID, $h$, $ek_f$}
    \Let{valid}{false}
  ...
  \Procedure{\sf process}{e}
    ...
    \Let{(\pi, y, valid)}{compute(d, algorithmID, h, ek_f)}
...
\begin{algorithm}[!htb]
  ...
  \Procedure{\sf verify}{e}
    ...
    \Let{result}{\_verify(\pi, y ,valid, vk_f)} \Comment{Verify proof}
